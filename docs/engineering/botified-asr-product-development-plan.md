...
- fake CAM++ embedding 固定一个未知人数样本，覆盖 float64 clustering 链、未 clip negative cosine、stable row-wise pruning 及 canonical-anchor tie、symmetrization/zero diagonal、absolute degree、scale-aware eigenvalue clamp、raw spectral `Y` 且不 row-normalize、低频 normalized-gap 筛选、并列取最小 `K`、deterministic 10-restart k-means 和 restart 全失败语义、绝对时间投影及相邻同标签合并；另覆盖 `N=0/1/2`、all-zero 和无合格候选均回退 `K=1`，以 §8.5 唯一算法的固定输出为准，并以超过 32 个匿名 cluster 的 synthetic embedding 验证实际 speaker 数无硬上限。
